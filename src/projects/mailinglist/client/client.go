...
func logResponse(res *pb.EmailResponse, err error) {
	...
	if res.EmailEntry == nil {
		...
		log.Printf("rrresponse %v", res.EmailEntry)
	}
}
...
var args struct {
	GrpcAddr string `arg:"env:MAILINGLIST_BIND_GRPC_ADDR"`
}
...
func main() {
	...
	newEmail := getEmail(client, "surya@example.com")
	//  GetEmailBatch(client, 5, 1)
	log.Printf("newEmail %v", *newEmail)
